$(function () {
    ...
    const tableBtnsMeta_studentManagement = {
        'names': ['重置密码', '导出名单'],
        'icons': ['reply-all', 'table'],
        'ids-suffix': ['resetPassword', 'export-excel'],
        'useableConditions': [2, 0],
    }
    ...
    function setListener() {
        ...
        $('#table-btn-export-excel').unbind('click').click(e => {
            const ids = table_studentManagement.tablelist.map(user => user.id).join(',');
            console.log(ids);
            action.exportStudentList('(' + ids + ')');
        });
    }
})