...
public class HomeworkService extends BaseServiceImpl<Homework> {
	...
	public void sumbitHomework(Integer userId, Integer homeworkId,
		...
		String sql = "";
		sql="SELECT * FROM ANNOTATION_HOMEWORK_STUDENT WHERE HOMEWORK_ID = " + homeworkId + "AND USER_ID = "+userId;
		int count = this.getTotalCountBySQL(sql);
		
		if(count<=0){
			sql = "INSERT INTO ANNOTATION_HOMEWORK_STUDENT (USER_ID, HOMEWORK_ID, SOURCE_LINK,STUDENT_HOMEWORK) VALUES ("
				+ userId
				+ ","
				+ homeworkId
				+ ",'"
			...
			sql = "UPDATE ANNOTATION_HOMEWORK_STUDENT SET SOURCE_LINK = ? , STUDENT_HOMEWORK = ? WHERE USER_ID = ? AND HOMEWORK_ID = ?";
			this.executeSQLUpdate(sql, sourceLink, stuContent, userId, homeworkId);
		}
	}
	...
	public List<Map<String, Object>> findTeaHomework(Integer teaId) {
		String sql = "SELECT HOMEWORK_ID AS homeworkId, HOMEWORK_NAME AS homeworkName, USER_ID,STUDENT_NUMBER, CREATION_TIME AS createtime, DEADLINE AS deadtime, CONVERT(text,CLASS_NAME) AS classesName FROM HOMEWORK_INFO WHERE USER_ID = "
				+ teaId + "ORDER BY createtime DESC";
	...
	public List<Map<String, Object>> submitHomeworkService(
		...
		try {
			...
			if(homeworkFile!=null){
				...
				path = user.getUserCode() + "/homework/" + my_fileName;
			}
	...
	public List<Map<String, Object>> stuSearchHomeWork(Integer stuId)
		...
		String sql = "SELECT DISTINCT HOMEWORK_ID AS homeworkId,TEA_ID AS userId,STU_ID AS stuId,HOMEWORK_NAME AS homeworkName,HOMEWORK_CONTENT AS homeworkContent,TEA_NAME AS teacherName,CREATION_TIME AS startTime,DEADLINE AS endTime,SOURCE_LINK AS sourceLink,EVALUATION AS evaluation,SCORE AS score,STUDENT_LINK AS submitLink,STUDENT_HOMEWORK_CONTENT AS studentHomeworkContent FROM STUDENT_HOMEWORK_VIEW WHERE STU_ID = "
				+ stuId+"ORDER BY startTime DESC";
		List<Map<String, Object>> temp = this.getResultBySQL(sql);
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				"yyyy-MM-dd HH:mm:ss");
		Integer Flag;
		for (Map<String, Object> i : temp) {
			...
			if(i.get("submitLink")=="" || i.get("submitLink")=="null"){
				i.put("studentLink", "");
				System.out.println("here,wozaizheli");
			}else{
